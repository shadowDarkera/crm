...
public class SaleVisitAction extends ActionSupport implements
		ModelDriven<SaleVisit> {
	...
	public String findAll() {
		...
		DetachedCriteria detachedCriteria = DetachedCriteria
				.forClass(SaleVisit.class);
		// 设置条件:
		if (saleVisit.getVisit_time() != null) {
			detachedCriteria.add(Restrictions.ge("visit_time",
					saleVisit.getVisit_time()));
		}
		if (visit_end_time != null) {
			detachedCriteria.add(Restrictions.le("visit_time", visit_end_time));
		}
		// 调用业务层:
		PageBean<SaleVisit> pageBean = saleVisitService.findByPage(
